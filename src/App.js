...
const HEALTH = {
  Healthy:  { bg: "border-[#B5DF07]/30 bg-[#B5DF07]/5" },
  "At Risk":{ color: "#facc15", bg: "border-yellow-500/30 bg-yellow-400/5" },
  Inactive: { color: "#f87171", bg: "border-red-500/30 bg-red-400/5" },
};
const TIER_BADGE = {
  ...
  return (
    <div className="min-h-screen text-gray-100 font-sans" style={{ backgroundColor: BG0 }}>
      ...
      <div className="border-b px-6 py-4 flex items-center justify-between"
        style={{ backgroundColor: BG1, borderColor: BRD }}>
        ...
        <div className="flex gap-3">
          {[
            ["Total",      stats.total,     "#d1d5db"],
            ["Healthy",    stats.healthy,   BRAND],
            ["At Risk",    stats.atRisk,    "#facc15"],
            ["Inactive",   stats.inactive,  "#f87171"],
            ["Open Tasks", stats.openTodos, "#d1d5db"],
          ].map(([l, v, col]) => (
            <div key={l} className="rounded-lg px-3 py-2 min-w-[64px] text-center"
        ...
        <div className="p-6">
            ...
            {["All", "Healthy", "At Risk", "Inactive"].map(f => (
              <button key={f} onClick={() => setFilter(f)}
                ...
                style={filter === f
                  ...
                  : { backgroundColor: BG2, color: "#9ca3af", border: "1px solid " + BG3 }}>
                {f}
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filtered.map(c => {
              ...
              const inactive  = c.workspace.totalProjects - c.workspace.activeProjects;
              const nextTodo  = c.todos.find(t => t.status !== "Done");
              const pct       = Math.round((c.workspace.activeProjects / c.workspace.totalProjects) * 100);
              return (
                <div key={c.id} onClick={() => { setSelected(c.id); setActiveTab("overview"); }}
                  className={"rounded-xl border cursor-pointer transition-all group " + HEALTH[h].bg}
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <div className="flex items-center gap-2">
                          <h2 className={"font-semibold text-white text-base group-hover:text-[" + BRAND + "] transition-colors"}>{c.name}</h2>
                      ...
                      {openCount > 0 && (
                        <div className="text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center text-black"
                          style={{ backgroundColor: BRAND }}>{openCount}</div>
                      )}
                    </div>
                    ...
                    <div className="rounded-lg p-3 mb-3" style={{ backgroundColor: BG2 }}>
                      <div className="text-[10px] text-gray-500 uppercase tracking-wide mb-1">
                        Last Activity · {c.mixpanel.daysAgo}d ago
                      </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-3">
                      {[["Projects", c.workspace.totalProjects], ["Active", c.workspace.activeProjects], ["Inactive", inactive]].map(([l, v]) => (
                    ...
                    <div className="mb-3">
                      <div className="flex justify-between text-[10px] text-gray-500 mb-1">
                        <span>Project Activity</span><span>{pct}% active</span>
                      </div>
                      <div className="h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: BG3 }}>
                      ...
                      <div className="rounded-lg p-2 text-xs" style={{ backgroundColor: BG2 }}>
                        ...
                        <div className="flex items-center gap-1.5">
                          <div className="w-1.5 h-1.5 rounded-full flex-shrink-0"
                            style={{ backgroundColor: PRIORITY[nextTodo.priority] }} />
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-1">
                <div className="w-2 h-2 rounded-full"
                  style={{ backgroundColor: healthColor(getHealth(client.mixpanel.daysAgo)) }} />
                <span className="text-sm font-medium"
                  style={{ color: healthColor(getHealth(client.mixpanel.daysAgo)) }}>
          ...
          <div className="flex gap-1 mb-5 rounded-lg p-1 w-fit" style={{ backgroundColor: BG2 }}>
            {["overview", "todos"].map(t => (
              <button key={t} onClick={() => setActiveTab(t)}
                ...
                {t === "todos"
                  ? "Tasks (" + client.todos.filter(td => td.status !== "Done").length + " open)"
                  : "Overview"}
              </button>
            ))}
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="rounded-xl p-5" style={{ backgroundColor: BG1, border: "1px solid " + BRD }}>
                ...
                <div className="space-y-3">
                  <div>
                    <div className="text-gray-500 text-xs mb-0.5">Last Event</div>
                    <div className="text-white font-semibold">{client.mixpanel.lastEvent}</div>
                  </div>
                  <div>
                    <div className="text-gray-500 text-xs mb-0.5">By User</div>
                    <div className="text-sm" style={{ color: BRAND }}>{client.mixpanel.lastUser}</div>
                  </div>
                  <div>
                    <div className="text-gray-500 text-xs mb-0.5">Date</div>
                    <div className="text-white text-sm">
                      {client.mixpanel.lastDate}{" "}
                      <span className="text-gray-500">({client.mixpanel.daysAgo} days ago)</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl p-5" style={{ backgroundColor: BG1, border: "1px solid " + BRD }}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ["Total Projects",   client.workspace.totalProjects],
                    ["Active Projects",  client.workspace.activeProjects],
                    ["Inactive (3mo+)",  client.workspace.totalProjects - client.workspace.activeProjects],
                    ["Members",          client.workspace.members],
                  ].map(([l, v]) => (
                ...
                <div className="mt-3">
                  ...
                  <div className="h-2 rounded-full overflow-hidden" style={{ backgroundColor: BG3 }}>
                    <div className="h-full rounded-full"
                      style={{ width: Math.round((client.workspace.activeProjects / client.workspace.totalProjects) * 100) + "%", backgroundColor: BRAND }} />
                  </div>
                </div>
              </div>
            </div>
          ...
          {activeTab === "todos" && (
            <div>
              <button onClick={() => openAdd(client.id)}
                className="mb-4 flex items-center gap-2 text-black text-sm font-medium px-4 py-2 rounded-lg hover:opacity-90 transition-all"
              ...
              <div className="space-y-3">
                {client.todos.length === 0 && (
                  <div className="text-gray-600 text-sm py-8 text-center">No tasks yet. Add one above.</div>
                )}
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-3 flex-1 min-w-0">
                        <div className="w-2.5 h-2.5 rounded-full flex-shrink-0 mt-1.5"
                          style={{ backgroundColor: PRIORITY[todo.priority] }} />
                      ...
                      <div className="flex gap-1 flex-shrink-0">
                        <button onClick={() => openEdit(client.id, todo)}
                        ...
                        <button onClick={() => deleteTodo(client.id, todo.id)}
      ...
      {todoModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="rounded-2xl w-full max-w-md p-6"
            style={{ backgroundColor: BG1, border: "1px solid " + BG3 }}>
            <h3 className="text-white font-semibold text-lg mb-4">
              {todoModal.todo ? "Edit Task" : "Add Task"}
            </h3>
            <div className="space-y-3">
              <div>
